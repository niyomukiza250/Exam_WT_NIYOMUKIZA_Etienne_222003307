...
<main>
    <h2>Currently Available Responses
    ...
    <table id="responseTable" class="table table-hover text-center">
        <tr>
            <th>Response ID</th>
            <th>Respondent</th>
            <th>Survey</th>
...
// Fetch data from the Responses, User, Surveys, Questions, and SurveyParticipant tables
$sql = "SELECT r.ResponseID, CONCAT(u.FirstName, ' ', u.LastName) as RespondentName, s.Title as SurveyTitle, q.QuestionText, r.Answer, r.ResponseDate, sp.ParticipationDate, sp.Status 
        FROM Responses r 
        JOIN User u ON r.RespondentID = u.UserID
        JOIN Surveys s ON r.SurveyID = s.SurveyID
        JOIN Questions q ON r.QuestionID = q.QuestionID
        JOIN SurveyParticipant sp ON r.RespondentID = sp.RespondentID AND r.SurveyID = sp.SurveyID";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . $row["ParticipationDate"] . "</td>";
        echo "<td>" . $row["Status"] . "</td>";
        echo "<td>"; // Actions column
        // Edit action
        echo "<a href='edit_participant.php?updateID=" . $row["ResponseID"] . "'><i class='fas fa-edit'></i></a>";
        // Delete action
        echo "<a href='delete_participant.php?deleteID=" . $row["ResponseID"] . "'><i class='fas fa-trash'></i></a>";
        echo "</td>";
        echo "</tr>";
    }
} else {
...
<script>
function printTable() {
    var divToPrint = document.getElementById("responseTable");
    var newWin = window.open("");
    newWin.document.write('<html><head><title>Print Response Records</title>');
