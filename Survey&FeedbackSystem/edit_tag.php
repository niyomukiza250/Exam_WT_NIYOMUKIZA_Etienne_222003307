...
<head>
    ...
    <title>Update Feedback Tag Record</title>
...
<body>
    <main>
        <div class="container">
            <h2>Update Feedback Tag Record</h2>
            <?php
            // Connection details
            ...
            if (isset($_GET['updateID']) && isset($_POST['submit'])) {
                ...
                $stmt = $conn->prepare("UPDATE FeedbackTag SET TagName=?, Description=? WHERE TagID=?");
                $stmt->bind_param("ssi", $tagName, $description, $id);
                ...
                try {
                    if ($stmt->execute()) {
                        ...
                        echo "<script>
                                if(confirm('$updateMessage')) {
                                    window.location.href = 'viewFeedbacktag.php';
                                }
                              </script>";
                    } else {
                    ...
                    echo "<script>
                            alert('Cannot update feedback tag record. Please check associated records and try again later.');
                            window.location.href = 'viewFeedbacktag.php';
                          </script>";
                }
            ...
            if (isset($_GET['updateID'])) {
                $id = $_GET['updateID'];
                $sql_select = "SELECT * FROM FeedbackTag WHERE TagID=?";
            ...
            <form method="POST" action="edit_tag.php?updateID=<?php echo $id; ?>">
                <input type="hidden" name="id" value="<?php echo $id; ?>">
                <div class="form-group">
                    <label for="tagName">Tag Name:</label>
                    <input type="text" id="tagName" name="tagName" value="<?php echo isset($row['TagName']) ? htmlspecialchars($row['TagName']) : ''; ?>" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <a href="viewFeedbacktag.php" class="btn btn-secondary">Cancel</a>
                </div>
            </form>
        </div>
    </main>
