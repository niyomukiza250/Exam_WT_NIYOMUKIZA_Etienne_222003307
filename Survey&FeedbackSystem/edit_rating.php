    ...
    <main>
        <div class="container">
            ...
            if (isset($_GET['updateID']) && isset($_POST['submit'])) {
                ...
                $stmt = $conn->prepare("UPDATE FeedbackRating SET ResponseID=?, RatingValue=?, RatingDate=?, RaterID=? WHERE RatingID=?");
                $stmt->bind_param("iissi", $responseID, $ratingValue, $ratingDate, $raterID, $id);
                ...
                try {
                    ...
                    echo "<script>
                            alert('Cannot update feedback rating record. Please check associated records and try again later.');
                            window.location.href = 'viewRating.php';
                          </script>";
                }
            ...
            if (isset($_GET['updateID'])) {
                $id = $_GET['updateID'];
                $sql_select = "SELECT * FROM FeedbackRating WHERE RatingID=?";
            ...
            <form method="POST" action="edit_rating.php?updateID=<?php echo $id; ?>">
                <input type="hidden" name="id" value="<?php echo $id; ?>">
                <div class="form-group">
                    <label for="responseID">Response ID:</label>
                    <input type="number" id="responseID" name="responseID" value="<?php echo isset($row['ResponseID']) ? $row['ResponseID'] : ''; ?>" required>
                </div>
                <div class="form-group">
                    <label for="ratingValue">Rating Value:</label>
                    <input type="number" id="ratingValue" name="ratingValue" value="<?php echo isset($row['RatingValue']) ? $row['RatingValue'] : ''; ?>" required>
                </div>
                <div class="form-group">
                    <label for="ratingDate">Rating Date:</label>
                    <input type="datetime-local" id="ratingDate" name="ratingDate" value="<?php echo isset($row['RatingDate']) ? date('Y-m-d\TH:i', strtotime($row['RatingDate'])) : ''; ?>" required>
                </div>
                <div class="form-group">
                    <label for="raterID">Rater ID:</label>
                    <input type="number" id="raterID" name="raterID" value="<?php echo isset($row['RaterID']) ? $row['RaterID'] : ''; ?>" required>
                </div>
                <div class="form-group">
                    <input type="submit" name="submit" value="Update" class="btn btn-primary">
                    <a href="viewRating.php" class="btn btn-secondary">Cancel</a>
                </div>
            </form>
        </div>
    </main>
