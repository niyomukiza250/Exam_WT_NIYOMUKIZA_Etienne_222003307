...
<head>
    ...
    <title>Feedback Rating Management</title>
...
<main>
    ...
    <br><br><br>
    <!-- Table for displaying feedback ratings -->
    <table id="ratingTable" class="table table-hover text-center">
        <tr>
            <th>Rating ID</th>
            <th>Response</th>
            <th>Rating Value</th>
        ...
        <?php
        // Fetch data from the FeedbackRating table
        $sql = "SELECT FeedbackRating.RatingID, Responses.Answer, 
                       FeedbackRating.RatingValue, FeedbackRating.RatingDate, User.UserName 
                FROM FeedbackRating
                JOIN Responses ON FeedbackRating.ResponseID = Responses.ResponseID
                JOIN User ON FeedbackRating.RaterID = User.UserID";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                // Edit action
                echo "<a href='edit_rating.php?updateID=" . $row["RatingID"] . "'><i class='fas fa-edit'></i></a>";
                // Delete action
                echo "<a href='delete_rating.php?deleteID=" . $row["RatingID"] . "'><i class='fas fa-trash'></i></a>";
                echo "</td>";
                echo "</tr>";
            }
        } else {
...
<div class="footer">
    &copy; 2024 Feedback Rating Management
</div>

<script>
function printTable() {
    var divToPrint = document.getElementById("ratingTable");
    var newWin = window.open("");
    newWin.document.write('<html><head><title>Print Feedback Rating Records</title>');
