...
<body>
    ...
    <button class="btn-print" onclick="printTable()">
        ...
        <table id="reportsTable" class="table table-hover text-center">
            <tr>
                <th>Report ID</th>
                <th>Report Name</th>
            ...
            <?php
            // Fetch data from the Reports table with creator name instead of ID
            $sql = "SELECT r.ReportID, r.ReportName, r.ReportDescription, u.UserName as CreatorName, r.CreationDate 
                    FROM Reports r 
                    JOIN User u ON r.CreatorID = u.UserID";
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>"; // Actions column
                     // Edit action
                     echo "<a href='edit_report.php?updateID=" . $row["ReportID"] . "'><i class='fas fa-edit'></i></a> ";
                     // Delete action
                     echo "<a href='delete_report.php?deleteID=" . $row["ReportID"] . "'><i class='fas fa-trash'></i></a>";
                    echo "</td>";
                    echo "</tr>";
                }
            } else {
    ...
    <script>
        function printTable() {
            var divToPrint = document.getElementById("reportsTable"); // Corrected ID
            var newWin = window.open("");
            newWin.document.write('<html><head><title>Print Survey Records</title>');
